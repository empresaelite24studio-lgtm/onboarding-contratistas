...
app.post('/api/send-email', async (req, res) => {
  ...
  if (!host || !user || !pass) {
    ...
    return res.status(500).json({ error: 'Configuración de SMTP incompleta en el archivo .env' });
  }
  ...
  try {
    const mailOptions = {
      ...
      to: user, // Send notification to HR / Admin email configured as SMTP_USER
    ...
    console.log(`[SMTP success] Onboarding email sent for: ${contractorName}`);
    res.json({ success: true });
  } catch (error) {
    console.error('[SMTP error] Failed to send email:', error);
    res.status(500).json({ error: 'Error al enviar el correo a través de SMTP', details: error.message });
  }
});

const PORT = process.env.PORT || 3001;
app.listen(PORT, () => {
  console.log(`Local onboarding-contratistas API running on http://localhost:${PORT}`);
});
